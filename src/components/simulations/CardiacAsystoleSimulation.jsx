  ...
  const steps = {
    ...
    'asystole-check': {
      ...
      options: [
        ...
        { text: 'C) Check for a pulse for $\le 10$ seconds.', isCorrect: false, nextStep: 'fail' }
      ]
    },
    'asystole-action': {
    ...
    'rosc-pulse-check': {
      ...
      message: `After the $CPR$ cycle, the team leader checks the pulse. A strong carotid pulse is felt, and the patient's $ETCO2$ suddenly jumps from $15\text{ mmHg}$ to $40\text{ mmHg}$. $ROSC$ is confirmed. What is the highest priority now?`,
      type: 'green',
      isDecision: true,
      options: [
        { text: 'A) Continue chest compressions until the patient wakes up.', isCorrect: false, nextStep: 'fail' },
        { text: <span>B) <span style={{fontWeight: 'bold'}}>Optimize Airway & Ventilation (Maintain $O_2$ saturation $\ge 94\%$ and manage $\text{CO}_2$).</span></span>, isCorrect: true, nextStep: 'post-arrest-bp' },
        { text: 'C) Administer a third dose of Epinephrine.', isCorrect: false, nextStep: 'fail' }
      ]
    },
    'post-arrest-bp': {
      title: '🏥 Post-Cardiac Arrest Care ($P\text{-}CACC$)',
      message: `The patient is stable but hypotensive ($BP\ 80/55\text{ mmHg}$). The team must manage hypotension aggressively to maintain organ perfusion. What is the target SBP (Systolic Blood Pressure)?`,
      type: 'green',
      isDecision: true,
      options: [
        { text: 'A) $SBP$ target $\ge 120\text{ mmHg}$.', isCorrect: false, nextStep: 'fail' },
        { text: <span>B) <span style={{fontWeight: 'bold'}}>$SBP$ target $\ge 90\text{ mmHg}$ (or $MAP \ge 65\text{ mmHg}$).</span></span>, isCorrect: true, nextStep: 'pass' },
        { text: 'C) $SBP$ target $70\text{ mmHg}$ to allow for passive warming.', isCorrect: false, nextStep: 'fail' }
      ]
    },
    'pass': {
      title: '✅ Simulation 3 Passed!',
      message: "Outstanding! You correctly navigated the non-shockable algorithm, recognized $ROSC$ using clinical signs ($ETCO2$ jump), and initiated the critical $P\text{-}CACC$ steps by managing airway and blood pressure. Proceed to the Post-Arrest Care Documentation review.",
      type: 'green',
      isDecision: false,
      actions: {
        confirm: () => onPass(), 
    ...
    'fail': {
      ...
      message: "A major error in $PEA$/Asystole management or $ROSC$ priority setting occurred. Review the non-shockable algorithm and $P\text{-}CACC$ steps, then try again.",
      type: 'red',
      isDecision: false,
      actions: {
        confirm: () => {
      ...
      <div className="scenario-container">
        ...
        <div className="patient-bedside-view">
          ...
          <div className="monitor-display">
            ...
            <p className="monitor-line monitor-rr">End-Tidal $\text{CO}_2$ ($ETCO2$): <span className={step === 'rosc-pulse-check' ? 'green-text' : ''}>$15 \to 40\text{ mmHg}$</span></p>
            <p className="monitor-line monitor-bp">BP: <span className={step === 'post-arrest-bp' ? 'pink-text' : ''}>$80/55\text{ mmHg}$ (Post-$ROSC$)</span></p>
          </div>
        </div>
